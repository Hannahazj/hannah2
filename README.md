...
export class ReportGenComponent extends BaseComponent{
  ...
  updateSectionType(section, type){
    ...
    // Only act for special templates
 ...
 */
   ...
   * FIX: use instance index only for content_array, not sections[]
   */
  updateInstancePosition(instanceId){

    const activeSection = this.report.sections[this.currentSection];
    const instanceIdx = activeSection.content_array.findIndex(obj => obj.id === instanceId);
    
    this.sectionSettingsForm.reset();

    if (activeSection?.section_settings){
      const section_settings = activeSection.section_settings;
    ...
    this.currentInstancePosition = instanceIdx;
  ...
  // ------------------- speaking_notes helpers & actions -------------------
  ...
  private buildSpeakingHeader(role: string, topic: string, dateCivilian: string) {
    // inline styles so we don't need SCSS changes
...
${dateCivilian}
  ...
  private buildSpeakingSections() {
    const keyPointsHTML = `<div style="font-family:Arial,sans-serif;font-size:12pt">
Key Discussion Points:
<ul style="margin:0 0 6pt 1.1em;padding:0">
  <li style="margin:0 0 3pt 0">Discussion point # 1
    <ul style="margin:0 0 6pt 1.1em;padding:0">
      <li style="margin:0 0 3pt 0"><strong>May bold important parts for emphasis.</strong></li>
      <li style="margin:0 0 3pt 0">Spacing between bullets should be 3pt.</li>
      <li style="margin:0 0 3pt 0">Spacing between sections should be 6pt.</li>
    </ul>
  </li>
  <li style="margin:0 0 3pt 0">Discussion point # 2</li>
  <li style="margin:0 0 3pt 0">● Solid bullet</li>
  <li style="margin:0 0 3pt 0">○ Hollow bullet
    <ul style="margin:0 0 6pt 1.1em;padding:0"><li style="margin:0 0 3pt 0">▪ Square bullet</li></ul>
  </li>
  <li style="margin:0 0 3pt 0">Discussion point # 3
    <ul style="margin:0 0 6pt 1.1em;padding:0"><li style="margin:0 0 3pt 0">Maintain Arial 11pt or 12pt throughout the document.</li></ul>
  </li>
  <li style="margin:0 0 3pt 0">Page numbers should identify the page number including the total number of pages.</li>
  <li style="margin:0 0 3pt 0">When consolidating several topics, ensure the numbers correspond with the topic not the entire file (i.e., 1 of 3 not 1 of 20).</li>
  <li style="margin:0 0 3pt 0">Discussion point # 4
    <ul style="margin:0 0 6pt 1.1em;padding:0"><li style="margin:0 0 3pt 0">The header should remain with the topic in blue with the Director or Vice Director and dates in black.</li></ul>
  </li>
  <li style="margin:0 0 3pt 0">Discussion point # 5
    <ul style="margin:0 0 6pt 1.1em;padding:0">
      <li style="margin:0 0 3pt 0">The dates are civilian formatting month, day, year.</li>
      <li style="margin:0 0 3pt 0">Always use Oxford comma rules.</li>
    </ul>
  </li>
</ul>
</div>`;

    const closingHTML = `<div style="font-family:Arial,sans-serif;font-size:12pt">
Closing Discussion Point:
<ul style="margin:0 0 6pt 1.1em;padding:0"><li style="margin:0 0 3pt 0">Final, “get off the stage” point.</li></ul>
</div>`;

    const avoidHTML = `<div style="font-family:Arial,sans-serif;font-size:12pt">
Topics to Avoid:
<ul style="margin:0 0 6pt 1.1em;padding:0">
  <li style="margin:0 0 3pt 0">Topic # 1 to avoid</li>
</ul>
</div>`;

    return [
      { name: 'Background', content: `<div style="font-family:Arial,sans-serif;font-size:12pt">Background: A brief description of the engagement and who is in the audience.</div>` },
