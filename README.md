...
@ViewChild('verticalViewport') viewport!: CdkVirtualScrollViewport;
  ...
  filter: string;
  sortColumn =  null; // no default sort column
  ...
  // added to track document for sorting
  trackByDoc = (_: number, row: any) =>  row.document_name ?? row.file_name ?? row.fiscal_year ?? row.last_updated;
  ...
  sort(colName, boolean) {
    ...
    private applySort() {
      ...
      this.viewport?.scrollToIndex(0, 'auto'); // Reset scroll position after sorting
...
}"  document list ocmponent.ts "import { AfterViewInit, Component, ViewChild, ElementRef, Input, ChangeDetectorRef } from '@angular/core';
